...
class Encoder(nn.Module):
    def __init__(self, hidden_channels, filter_channels, n_heads, n_layers, kernel_size=1, p_dropout=0., window_size=4,
                 **kwargs):
        super().__init__()
        self.hidden_channels = hidden_channels          # 隐藏层 192
        self.filter_channels = filter_channels          # 滤波层 768
        ...
        self.ffn_layers = nn.ModuleList()               # 前馈网络FFN
        self.norm_layers_2 = nn.ModuleList()
        for i in range(self.n_layers):                  # 多层
            self.attn_layers.append(MultiHeadAttention(hidden_channels, hidden_channels, n_heads, p_dropout=p_dropout,
                                                       window_size=window_size))    # attn层
            self.norm_layers_1.append(LayerNorm(hidden_channels))                   # 正则层
...
class MultiHeadAttention(nn.Module):
    def __init__(self, channels, out_channels, n_heads, p_dropout=0., window_size=None, heads_share=True,
        ...
        self.conv_q = nn.Conv1d(channels, channels, 1)      # 通过一维卷积转成conv_q
...
class FFN(nn.Module):
    def __init__(self, in_channels, out_channels, filter_channels, kernel_size, p_dropout=0., activation=None,
                 causal=False):
        super().__init__()
        self.in_channels = in_channels          # 输入
        self.out_channels = out_channels        # 输出
        self.filter_channels = filter_channels  # 滤波通道
        self.kernel_size = kernel_size
        self.p_dropout = p_dropout
